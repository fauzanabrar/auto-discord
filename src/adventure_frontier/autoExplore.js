function loopExploreLastPlace(times = 5, option_index = 10, delay = 5500) {
    let count = 0;

    const interval = setInterval(() => {
        const dropdown = [
            ...document.querySelectorAll('div[role="button"]'),
        ].find((div) => div.textContent.trim() === "Choose a place...");
        ...
        setTimeout(() => {
            ...
            if (container) {
                options = container.querySelectorAll(
                    '[data-list-item-id*="___choose-explore-"]'
                );
            } else {
            ...
            const lastOption =
                options[options.length - 1 - (10 - option_index)];
            ...
            console.log(
                `Selected last place [${
                    count + 1
                }/${times}]: ${lastOption.textContent.trim()}`
            );
            ...
            setTimeout(() => {
                ...
                ].find((btn) => btn.textContent.trim() === "Explore");
